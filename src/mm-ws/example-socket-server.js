        ...
        if (req.headers['upgrade'].toLowerCase() === 'websocket') {
            ...
            if (version === '13') {
                ...
                socket.on('data', function(data) {
                    ...
                    dataReceived = Buffer.concat(
                        [dataReceived, data],
                        dataReceived.length + data.length
                    );
                    ...
                    while (true) {
                        ...
                        if ((dataReceived.readUInt8(1) & 127) < 126) {
                            ...
                            payloadLength =
                                (dataReceived.readUInt32BE(2) << 32) +
                                dataReceived.readUInt32BE(6);
                            nextByteIndex += 9;
                        }
                        ...
                        if (mask) {
                            for (var i = 0; i < payloadLength; i += 1) {
                                dataReceived.writeUInt8(
                                    dataReceived.readUInt8(nextByteIndex + i) ^
                                        maskingKey.readUInt8(i % 4),
                                    nextByteIndex + i
                                );
                            }
                        }
                        ...
                        payloadReceived = Buffer.concat(
                            ...
                            payloadReceived.length + payloadLength
                        );
                        nextByteIndex += payloadLength;
