...
const mm_date_1 = require("./mm-date");
class MMDateFormatter {
    ...
    static format(date, mask, locale = 'en', utc = false, i18n) {
        if (!mm_date_1.mmIsValidDate(date)) {
        ...
        keys.forEach((k) => patterns.push(mm_string_1.mmEscapeRegExp(k)));
    ...
    static from(date, compareTo, locale = 'en', i18n) {
        ...
        if (!mm_date_1.mmIsValidDate(compareTo)) {
