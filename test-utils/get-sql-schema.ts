...
export const getSqlSchema = (schemaDir) => {
    ...
    let sqlFiles = fs.readdirSync(schemaDir).filter((name) => path.extname(name) === '.sql');
    ...
    let sqlSchema;
    try {
        sqlSchema = resolve(sqlDepsGraph).reduce((acc, name) => (
